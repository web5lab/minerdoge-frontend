...
import { friendsSelector, RankSelector } from "../selector/globalSelector";
...
function Friends() {
  ...
  return (
    <div className="w-full h-full max-h-minus-60 flex px-4 flex-col items-center justify-center">
      <h1 className=" font-bold text-3xl mt-8">{friends?.length} Friends</h1>
      ...
      <div className="w-full bg-gray-700 rounded-lg flex flex-col my-4 gap-3 justify-center items-center p-2">
        ...
        }} className=" font-bold text-xl">Open Details</h1>
      </div>
      ...
      <div className="w-full h-full  overflow-auto   my-2">
        <div className=" w-full grid gap-3">
          {friends.map((data, index) => (
            <div key={index} className="flex w-full justify-start gap-3 items-center">
              <img className=" h-12 rounded-full" src="userlogo.png" />
              <div>
                <span className=" font-semibold">{data?.name}</span>
                <div className="flex justify-center items-center gap-2">
                  <img className=" h-6 rounded-full" src={rank[Number(data?.rank -1)]?.imageUrl} />
                  <span>{rank[Number(data?.rank -1)]?.tittle}</span>
                  <div className=" rounded-full h-5 w-[1px] bg-gray-300"></div>
                  <span className=" text-yellow-400 font-bold text-base">+ {rank[Number(data?.rank -1)]?.RefralAmount}</span>
                </div>
